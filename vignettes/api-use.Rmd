...
Several duplicate values... shown in

```{r duplicates}

head(rem$duplicate_variableIds, 5)

```
In case of duplicate variables, all data is downloaded. 
AFAIK numbrevalues are identical for duplicated variables, just differ in one/more ccmug id's.
Check and clean data afterwards to only include wanted obs..
Will develop function ..
